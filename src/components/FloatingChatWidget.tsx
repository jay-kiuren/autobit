...
const FB_USERNAME = "autobitofficial"; // your Facebook page username
...
// ─── Shared close button ───────────────────────────────────────────────────────
const PanelCloseBtn = ({ onClose, label }: { onClose: () => void; label: string }) => (
  <button
    onClick={onClose}
  ...
  return (
    <div className="flex flex-col rounded-2xl overflow-hidden shadow-2xl" style={{
      ...
      <div className="flex items-center gap-3 px-4 py-3 flex-shrink-0"
        ...
        <PanelCloseBtn onClose={onClose} label="Close WhatsApp chat" />
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-3 py-3 flex flex-col gap-2"
        ...
        <div className="flex justify-start">
          <div className="text-xs text-white/80 px-3 py-2 rounded-2xl rounded-tl-sm max-w-[85%] leading-relaxed"
      ...
      <div className="flex items-center gap-2 px-3 py-2.5 flex-shrink-0"
        ...
        <button onClick={send} aria-label="Send on WhatsApp"
          ...
          style={{ background: message.trim() ? "linear-gradient(135deg, #25D366 0%, #128C7E 100%)" : "rgba(255,255,255,0.08)" }}>
          <SendIcon />
        </button>
      </div>
...
function MessengerPanel({ onClose }: { onClose: () => void }) {
  ...
  const send = () => {
    const text = message.trim() || "Hello! I'd like to know more about AUTOBIT.";
    window.open(`https://m.me/${FB_USERNAME}?text=${encodeURIComponent(text)}`, "_blank", "noopener,noreferrer");
    setMessage("");
  };
  ...
  const quickMessages = [
    "Hi! I'd like to learn more about your services.",
    "Can I get a project quote?",
    "I'm interested in robotics or AI.",
  ];
  ...
  return (
    <div className="flex flex-col rounded-2xl overflow-hidden shadow-2xl" style={{
      ...
      background: "#111",
      border: "1px solid rgba(255,255,255,0.08)",
      boxShadow: "0 8px 40px rgba(0,0,0,0.6), 0 0 0 1px rgba(160,51,255,0.15)",
    }}>
      {/* Header */}
      <div className="flex items-center gap-3 px-4 py-3 flex-shrink-0"
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-1.5 mt-0.5">
            ...
            <p className="text-blue-200 text-xs">Typically replies quickly</p>
          </div>
        </div>
        <PanelCloseBtn onClose={onClose} label="Close Messenger chat" />
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-3 py-3 flex flex-col gap-2"
        ...
        <div className="flex justify-start">
          <div className="text-xs text-white/80 px-3 py-2 rounded-2xl rounded-tl-sm max-w-[85%] leading-relaxed"
            style={{ background: "rgba(255,255,255,0.08)", border: "1px solid rgba(255,255,255,0.06)" }}>
            👋 Hi! Message us on Messenger and we'll reply as soon as we can.
            <span className="block text-right text-white/30 text-[10px] mt-1">AUTOBIT</span>
          </div>
        </div>
        <div className="flex flex-col gap-1.5 mt-1">
          {quickMessages.map((msg) => (
            <button key={msg} onClick={() => setMessage(msg)}
              className="text-left text-xs px-3 py-2 rounded-xl transition-all duration-150 hover:scale-[1.02] active:scale-95"
              style={{
                background: message === msg ? "rgba(0,132,255,0.2)" : "rgba(0,132,255,0.07)",
                border: `1px solid ${message === msg ? "rgba(0,132,255,0.5)" : "rgba(0,132,255,0.2)"}`,
                color: "#90c8ff",
              }}>
              {msg}
            </button>
      ...
      <div className="flex items-center gap-2 px-3 py-2.5 flex-shrink-0"
        ...
        <input ref={inputRef} type="text" value={message}
          ...
          placeholder="Type a message..."
          className="flex-1 text-xs text-white placeholder-white/30 bg-transparent outline-none min-w-0" />
        <button onClick={send} aria-label="Send on Messenger"
          className="w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 transition-all duration-150 hover:scale-110 active:scale-95"
          style={{ background: message.trim() ? "linear-gradient(135deg, #0084FF 0%, #A033FF 100%)" : "rgba(255,255,255,0.08)" }}>
          <SendIcon />
        </button>
      </div>
      ...
      <div className="flex items-center justify-center gap-1.5 py-1.5 flex-shrink-0"
        ...
        <svg viewBox="0 0 48 48" fill="none" className="w-3 h-3">
          <defs>
            <radialGradient id="footerGrad" cx="19.25%" cy="99.46%" r="108.96%">
              <stop offset="0%"      stopColor="#09F" />
              <stop offset="60.975%" stopColor="#A033FF" />
              <stop offset="100%"   stopColor="#FF7061" />
            </radialGradient>
          </defs>
          <path d="M24 4C12.954 4 4 12.507 4 23.021c0 5.689 2.386 10.818 6.245 14.51.326.295.525.713.532 1.156l.108 3.612a1.27 1.27 0 0 0 1.783 1.12l4.028-1.778a1.27 1.27 0 0 1 .847-.062A21.39 21.39 0 0 0 24 42.042c11.046 0 20-8.507 20-19.021C44 12.507 35.046 4 24 4Z" fill="url(#footerGrad)"/>
        </svg>
        <span className="text-white/20 text-[10px]">Powered by Messenger</span>
      </div>
    </div>
  );
}
...
export default function FloatingChatWidget() {
  ...
  const closeFAB = () => {
    ...
    if (messengerOpen) { setMessengerOpen(false);  return; }
    setIsOpen((o) => !o);
  };
    ...
    <>
      ...
      {(isOpen || anyPanelOpen) && (
        <div className="fixed inset-0 z-40 sm:hidden"
          onClick={() => { setIsOpen(false); setWhatsappOpen(false); setMessengerOpen(false); }} />
      )}
      ...
      <div className="fixed bottom-6 right-5 z-50 flex flex-col items-end gap-3">
        ...
        {/* Messenger panel — same position, same animation as WhatsApp */}
        <div style={{
          opacity:        messengerOpen ? 1 : 0,
          transform:      messengerOpen ? "translateY(0) scale(1)" : "translateY(20px) scale(0.95)",
          transition:     "opacity 0.25s ease, transform 0.3s cubic-bezier(0.34,1.56,0.64,1)",
        ...
        {/* Sub-buttons */}
        <div className="flex flex-col items-end gap-3" style={{
          ...
          <div className="flex items-center gap-2"
            ...
            <div className="bg-[#1a1a1a] border border-white/10 text-white/70 text-xs font-medium px-3 py-1.5 rounded-full shadow-md transition-opacity duration-200 select-none"
              style={{ opacity: isWhatsappHovered ? 1 : 0 }}>
              WhatsApp
            </div>
            <button onClick={handleWhatsAppClick} aria-label="Chat on WhatsApp"
              ...
              style={{ background: "linear-gradient(135deg, #25D366 0%, #128C7E 100%)", boxShadow: "0 4px 20px rgba(37,211,102,0.4)" }}>
              <WhatsAppIcon />
            </button>
          </div>
          ...
          <div className="flex items-center gap-2"
            ...
            <button onClick={handleMessengerClick} aria-label="Chat on Messenger"
              className="w-12 h-12 rounded-full flex items-center justify-center shadow-lg hover:scale-110 active:scale-95 transition-transform duration-150"
              style={{ background: "radial-gradient(circle at 20% 100%, #09F 0%, #A033FF 61%, #FF5280 93%, #FF7061 100%)", boxShadow: "0 4px 20px rgba(160,51,255,0.4)" }}>
              <MessengerIcon />
            </button>
          </div>
        </div>
        ...
        <div className="relative">
          ...
          <button onClick={closeFAB}
            ...
            style={{
              background:  isOpen || anyPanelOpen ? "hsl(240 3% 20%)" : "linear-gradient(135deg, hsl(211 100% 58%) 0%, hsl(220 100% 46%) 100%)",
              boxShadow:   isOpen || anyPanelOpen ? "0 4px 24px rgba(0,0,0,0.5)" : "0 4px 24px hsl(211 100% 58% / 0.5)",
              border:      isOpen || anyPanelOpen ? "1px solid rgba(255,255,255,0.1)" : "none",
            }}>
            <div style={{ transition: "transform 0.3s cubic-bezier(0.34,1.56,0.64,1)", transform: isOpen || anyPanelOpen ? "rotate(45deg)" : "rotate(0deg)" }}>
              {isOpen || anyPanelOpen ? (
